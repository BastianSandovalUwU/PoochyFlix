...
export const rescanVideos = async (req: Request, res: Response): Promise<void> => {
  try {
    await videoService.rescanVideos();
    const videos = videoService.getAllVideos();
    
    res.json({
      success: true,
      data: videos,
      message: `Rescaneo completado. ${videos.length} videos encontrados.`
    });
  } catch (error) {
    console.error('Error en rescanVideos:', error);
    res.status(500).json({
      success: false,
      error: 'Error rescanando videos'
    });
  }
};